...
class AuthenticationMiddleware:
    ...
    def __init__(
        ...
        app: ASGIApp,
        tokens: list[str],
        unauthenticated_paths: frozenset[str] | None = None,
    ) -> None:
        self.app = app
...
def validate_plugin_path(plugin_path: str, plugin_type: str = "plugin") -> None:
    """
    Validate a plugin path to prevent path traversal and other security issues.

    This function checks that:
    1. The path does not contain ".." (path traversal attempts)
    2. The path is either absolute or a registered plugin name
    3. If the path looks like a file path, it must exist

    Args:
        plugin_path: The path to validate (e.g., "/path/to/plugin.py" or "registered_name")
        plugin_type: The type of plugin for error messages (e.g., "tool parser", "reasoning parser")

    Raises:
        ValueError: If the path fails validation
    """
    if not plugin_path or not isinstance(plugin_path, str):
        raise ValueError(f"Invalid {plugin_type} path: must be a non-empty string")

    # Check for path traversal attempts
    if ".." in plugin_path:
        raise ValueError(
            f"Invalid {plugin_type} path: path traversal ('..') is not allowed. "
            f"Path: {plugin_path}"
        )

    # Check if it looks like a file path (contains / or \ or ends with .py)
    is_file_path = "/" in plugin_path or "\\" in plugin_path or plugin_path.endswith(".py")
    ...
    if is_file_path:
        ...
        if not os.path.isabs(plugin_path):
            raise ValueError(
                f"Invalid {plugin_type} path: file paths must be absolute. "
                f"Path: {plugin_path}"
            )
        ...
        if not os.path.exists(plugin_path):
            raise ValueError(
                f"Invalid {plugin_type} path: file does not exist. "
                f"Path: {plugin_path}"
            )
        ...
        if not os.path.isfile(plugin_path):
            raise ValueError(
                f"Invalid {plugin_type} path: path must be a file, not a directory. "
                f"Path: {plugin_path}"
            )
    else:
        # Validate as a registered plugin name
        # Plugin names should be alphanumeric with underscores and hyphens
        if not re.match(r"^[a-zA-Z0-9_-]+$", plugin_path):
            raise ValueError(
                f"Invalid {plugin_type} name: must contain only alphanumeric characters, "
                f"underscores, and hyphens. Name: {plugin_path}"
            )
